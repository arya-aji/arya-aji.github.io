<script lang="ts">
  ...
  const messages: Record<number, { title: string; desc: string; hint: string }> = {
    404: {
  ...
  let msg = $derived(messages[status] ?? {
  ...
  let lines = $derived([
    `$ curl https://arya.dev${$page.url?.pathname ?? '/???'}`,
    `> HTTP/${status}`,
    `> X-Error: ${msg.title}`,
...
<main class="error-main">
  <div class="container">
    ...
    <div class="error-content">
      ...
      <div class="terminal-block" role="complementary" aria-label="Error details">
...
<style>
  ...
  @keyframes glitch-main {
    ...
    95% { transform: skewX(-2deg); filter: blur(0.4px); }
  ...
  @keyframes glitch-before {
    ...
    94%, 97% { transform: translateX(-4px); opacity: 0.8; }
  ...
  .t-cmd     { color: var(--ctp-text); }
  .t-header  { color: var(--ctp-subtext0); }
  .t-body    { color: var(--ctp-subtext1); }

  .t-cursor-line { opacity: 1; animation: none; margin-top: 4px; }
  .t-blink {
    color: var(--accent);
